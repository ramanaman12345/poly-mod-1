...
async function main() {
  try {
    ...
    const spaceNFT = process.env.CONTRACT_ADDR;
    const NFT = await ethers.getContractFactory("Space");
    const nft = NFT.attach(spaceNFT)
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const depositTx = await fxPortalBridge.connect(signer).deposit(spaceNFT, walletAddress, i, "0x6770",{ gasPrice });
